...
void print(char tbl_char[], int tbl_int[], double tbl_double[])
{
    ...
    for (index = 0;index<SIZE; index++)
    {
        printf(" %d \t tbl_char value %c and address: %x \n", index, tbl_char[index], &tbl_char[index]);
        printf(" %d \t tbl_int value %d and address: %x\n", index, tbl_int[index], &tbl_int[index]);
        printf(" %d \t tbl_double value %.3f and address:  %x\n", index, tbl_double[index], &tbl_double[index]);
    }
}